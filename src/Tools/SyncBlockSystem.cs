...
namespace ARTZone
{
    ...
    public partial class SyncBlockSystem : GameSystemBase
    {
        ...
        public struct SyncBlockJob : IJobParallelFor
        {
            ...
            {
                // --- gather core components safely ---
                Entity blockEntity = Entities[index];

                if (!BlockLookup.HasComponent(blockEntity) ||
                    !ValidAreaLookup.HasComponent(blockEntity))
                {
                    return;
                }
                ...
                if (settings != null)
                {
                    if (settings.RemoveOccupiedCells &&
                        IsAnyCellOccupied(cells, block, validArea))
                    {
                        return;
                    }

                    if (settings.RemoveZonedCells &&
                        IsAnyCellZoned(cells, block, validArea))
                    {
                        return;
                    }
                }

                // --- apply depth to block + valid area ---
